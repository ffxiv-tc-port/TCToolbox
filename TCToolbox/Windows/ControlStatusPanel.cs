...
/// <remarks>
/// <para>
/// 📌 <b>為什麼掛在急停底下</b>：「誰正壓著自動點擊／誰正在導航」這個問題，
/// 使用者會問它的時刻幾乎都是「我要不要按急停」或「我按了急停，怎麼還在動」。
/// 做成獨立模組的話它預設是關的，而一個預設關掉的<b>唯讀</b>顯示等於不存在。
/// </para>
/// <para>
...
/// </para>
/// <para>
/// 🔴 <b>這裡是 ImGui 的 Draw 路徑，不得擲例外。</b>狀態是靠 IPC 問來的，
/// 而 IPC 的失敗形式不只一種（見 <see cref="FleetControlStatus"/>）。
/// 整段包一層 try：撞到沒預期的例外就退化成一行灰字，不要把整扇視窗賠進去。
/// </para>
/// </remarks>
