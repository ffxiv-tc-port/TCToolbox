...
public sealed unsafe class AutoFCWSDeliver : TcModule
{
    ...
    private void OnMenuFinalize(AddonEvent type, AddonArgs args)
    {
        ...
        // ⚠️ 單項交納後這個 addon 其實「不會」被重建（整趟只在最後
    ...
    private void EnqueueNextItem()
    {
        queue.Enqueue("解析可交納素材", () =>
        {
            ...
            // 🔴 崩潰的另一半：上一項的確認框遲到彈出時，這裡照樣送下一項的合成事件，
            ...
            // 🔴 使用者回報「繳交太慢」的根因：指紋原本只有 (Index, Count)——
    ...
    private bool? ConfirmRequest()
    {
        ...
        if (yesno != null)
        {
            ...
            // 🔴🔴 這裡以前<b>完全不做文字判定</b>（註解寫「這個時點的確認框就是這條流程的」）。
            //    改成白名單有兩個理由，第二個才是主要的：
        ...
        //    ——「本模組按下 0 扇」正是靠這行才看得出來。
        // 🔴 文案修正：第三種情況以前寫「確認框未出現」，那是<b>本模組推不出來的斷言</b>——
